...
function retrieve_password() {
	...
	$message .= '<' . network_site_url( "backend.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . ">\r\n";
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'backend.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			<?php
			/**
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'backend.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit();
			}
		}
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'backend.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
			<p>
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( 'backend.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( 'backend.php?action=lostpassword&error=invalidkey' ) );
			}
		...
		?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'backend.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( 'backend.php?registration=disabled' ) );
			exit();
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'backend.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit();
			}
		}
		...
		?>
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'backend.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			<p>
	...
	case 'confirmaction':
		 ...
		 *
		 * After firing this action hook the page will redirect to backend a callback
	...
	default:
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			if ( $interim_login ) {
				...
				/** This action is documented in backend.php */
				do_action( 'login_footer' );
			...
			if ( is_a( $user, 'WP_User' ) && $user->exists() && $user->has_cap( 'manage_options' ) ) {
				...
				/** This filter is documented in backend.php */
				$admin_email_check_interval = (int) apply_filters( 'admin_email_check_interval', 6 * MONTH_IN_SECONDS );
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'backend.php', 'login_post' ) ); ?>" method="post">
			<p>
				<label for="user_login"><?php _e( 'Username or Email Address' ); ?></label>
